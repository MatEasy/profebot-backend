...
public class SimplifyService {
  ...
  private NodeStatus searchFunction(SearchMode mode, TreeNode node) {
  ...
  private NodeStatus executeSearchFunction(SearchMode mode, TreeNode node) {
    switch (mode) {
      case BASIC_SEARCH:
  ...
  protected NodeStatus evaluateConstantSum(TreeNode node) {
    ...
    if (nodeStatus.hasChanged() && TreeUtils.isConstantOrConstantFraction(nodeStatus.getNewNode())) {
      return nodeStatus;
    }
  ...
  private boolean canAddLikeTermNodes(TreeNode node, String termSubclass) {
    ...
    for (TreeNode child : args) {
      if (POLYNOMIAL_TERM.equals(termSubclass)) {
        ...
        if (!TreeUtils.isNthRootTerm(child)) {
          return false;
        }
      } else {
        return false;
  ...
  public boolean canMultiplyLikeTermPolynomialNodes(TreeNode node) {
    ...
    List<TreeNode> args = node.getArgs();
    for (TreeNode n : args) {
      if (!TreeUtils.isPolynomialTerm(n)) {
        return false;
      }
    }
    if (args.size() == 1) {
