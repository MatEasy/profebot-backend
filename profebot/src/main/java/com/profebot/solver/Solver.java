...
public class Solver {
  ...
  private List<EquationStatus> solveConstantEquation(Tree equation) {
    return solveConstantEquation(equation, new ArrayList<EquationStatus>());
  }
  ...
  protected List<EquationStatus> addSimplificationSteps(List<EquationStatus> steps, Tree equation) {
    ...
    Tree oldEquation = equation.clone();
         ...
         */
    List<NodeStatus> leftSimplifySteps = (new SimplifyService()).stepThrough(equation.getLeftNode());
    ...
    List<NodeStatus> rightSteps = (new SimplifyService()).stepThrough(equation.getRightNode());
  ...
  protected List<EquationStatus> solveConstantEquation(Tree equation, List<EquationStatus> steps) {
  ...
  private TreeNode getSolutionsAndSymbol(Tree equation, List<TreeNode> solutions){
    ...
    for (TreeNode factor :factorsWithSymbols) {
      ...
      steps = stepThrough(new Tree(leftNode, equation.getRightNode(), "="));
  ...
  protected List<EquationStatus> stepThrough(Tree equation){
    ...
    do {
      ...
      if (!equation.hasAnySymbol()) {
        return solveConstantEquation(equation, steps);
      }
  ...
  public String resolveExpression(String expression) throws InvalidExpressionException {
    ...
    Tree tree = (new Parser()).parseExpression(expression);

    List<EquationStatus> steps = stepThrough(tree);
