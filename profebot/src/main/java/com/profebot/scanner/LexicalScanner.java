...
public class LexicalScanner {
  ...
  private final int[][] tablaTransicion =
      {
          ...
          {99, 99, 60, 99, 60, 60, 60, 60, 60, 20, 99, 60, 60, 60},    /*  Estado 11 */
  ...
  private Token scanner() throws InvalidExpressionException {
    ...
    do {
      ...
      tipo = getType(c);
      if (tipo == -1) {
        throw new InvalidExpressionException("Detectado caracter invalido: " + c);
      }
      ...
      if (tipo == MENOS_CHAR && resultado.equals("")) {
        if (Token.CONSTANTE.equals(previousToken) ||
          ...
          estado = Acceptors.ST_ACEPTA_MENOS.getId();
      ...
      } else if (estado == Acceptors.ST_ACEPTA_FIN.getId()) {
        if (numParantesisAbiertos != 0) {
          throw new InvalidExpressionException(
              "Termina de parsear la expres�n, luego del = tiene " + numParantesisAbiertos + " parentesis abiertos!");
        }
      } else if (estado == Acceptors.ST_ACEPTA_PARENTESIS_IZQ.getId()) {
        numParantesisAbiertos++;
      } else if (estado == Acceptors.ST_ACEPTA_PARENTESIS_DER.getId()) {
        numParantesisAbiertos--;
        if (numParantesisAbiertos < 0) {
      ...
      if (estado == Acceptors.ST_EXPRESION_INVALIDA.getId()) {
  ...
  private boolean esAceptorSeparadorMiembros(int estado) {
    return estado == Acceptors.ST_ACEPTA_IGUAL.getId() ||
        estado == Acceptors.ST_ACEPTA_MENOR.getId() ||
        estado == Acceptors.ST_ACEPTA_MENOR_IGUAL.getId() ||
        estado == Acceptors.ST_ACEPTA_MAYOR.getId() ||
        estado == Acceptors.ST_ACEPTA_MAYOR_IGUAL.getId();
  }
  ...
  private int getType(char c) {
    ...
    if (Character.isDigit(c)) {
      ...
      typeC = PARENTESIS_IZQ_CHAR;
    } else if (c == ')') {
      typeC = PARENTESIS_DER_CHAR;
    } else if (c == '+') {
      typeC = MAS_CHAR;
    } else if (c == '-') {
      typeC = MENOS_CHAR;
    } else if (c == '*') {
      typeC = ASTERISCO_CHAR;
    } else if (c == '/') {
      typeC = BARRA_CHAR;
    } else if (c == '^') {
      typeC = POTENCIA_CHAR;
  ...
  private boolean estadoAceptor(int estado) {
    return estado >= Acceptors.INICIO_ST_ACEPTORES.getId();
  }

  private Token getToken(int estado) {

    Token t = null;
    if (estado == Acceptors.ST_ACEPTA_NUMERO.getId() || estado == Acceptors.ST_ACEPTA_NUMERO_NEGATIVO.getId()) {
      t = Token.CONSTANTE;
    } else if (estado == Acceptors.ST_ACEPTA_X_CON_COEF.getId() || estado == Acceptors.ST_ACEPTA_X_SIN_COEF.getId()
        || estado == Acceptors.ST_ACEPTA_X_CON_COEF_NEGATIVO.getId() || estado == Acceptors.ST_ACEPTA_X_SIN_COEF_NEGATIVO.getId()
        || estado == Acceptors.ST_ACEPTA_X_CON_COEF_Y_EXP.getId() || estado == Acceptors.ST_ACEPTA_X_SIN_COEF_Y_EXP.getId()
        || estado == Acceptors.ST_ACEPTA_X_CON_COEF_NEGATIVO_Y_EXP.getId() ||
        estado == Acceptors.ST_ACEPTA_X_SIN_COEF_NEGATIVO_Y_EXP.getId()) {
      t = Token.LETRA_X;
    } else if (estado == Acceptors.ST_ACEPTA_PARENTESIS_IZQ.getId()) {
      t = Token.PARENIZQUIERDO;
    } else if (estado == Acceptors.ST_ACEPTA_PARENTESIS_DER.getId()) {
      t = Token.PARENDERECHO;
    } else if (estado == Acceptors.ST_ACEPTA_MAS.getId()) {
      t = Token.SUMA;
    } else if (estado == Acceptors.ST_ACEPTA_MENOS.getId()) {
      t = Token.RESTA;
    } else if (estado == Acceptors.ST_ACEPTA_PRODUCTO.getId()) {
      t = Token.MULTIPLICACION;
    } else if (estado == Acceptors.ST_ACEPTA_DIVISION.getId()) {
      t = Token.DIVISION;
    } else if (estado == Acceptors.ST_ACEPTA_IGUAL.getId() ||
        estado == Acceptors.ST_ACEPTA_MENOR.getId() ||
        estado == Acceptors.ST_ACEPTA_MENOR_IGUAL.getId() ||
        estado == Acceptors.ST_ACEPTA_MAYOR.getId() ||
        estado == Acceptors.ST_ACEPTA_MAYOR_IGUAL.getId()
      ...
      t = Token.IGUAL;
    } else if (estado == Acceptors.ST_ACEPTA_POTENCIA.getId()) {
      t = Token.POTENCIA;
    } else if (estado == Acceptors.ST_ACEPTA_RAIZ.getId()) {
      t = Token.RAIZ;
    } else if (estado == Acceptors.ST_ACEPTA_FIN.getId()) {
      t = Token.FIN;
    }
